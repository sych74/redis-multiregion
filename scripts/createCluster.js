var regions = '${settings.regions}'.split(','), masterNodesString = "", createClusterCommand = "", resp, envInfo, announceIp, first
    ...
    rebalanceCommand = "export REDISCLI_AUTH=$(cat ${globals.redisConfigPath} |grep '^requirepass'|awk '{print $2}'); redis-cli --cluster check 127.0.0.1:6379 || redis-cli --cluster fix 127.0.0.1:6379; redis-cli --cluster rebalance 127.0.0.1:6379"
...
createClusterCommand = "export REDISCLI_AUTH=$(cat ${globals.redisConfigPath} |grep '^requirepass'|awk '{print $2}'); yes yes | redis-cli --cluster create  " + masterNodesString + " --cluster-replicas 0";
...
for (var cluster = 1, n = regions.length + 1; cluster < n; cluster++) {
    ...
    for (var i = 1, k = resp.nodes; i < k.length; i++) {
        if (k[i].nodeGroup == 'nosqldb') {
            ...
            if (resp.result != 0) { return resp; }
            announceIp = resp.responses[0].out
            var meetCommand = "export REDISCLI_AUTH=$(cat ${globals.redisConfigPath} |grep '^requirepass'|awk '{print $2}'); redis-cli -h 127.0.0.1 cluster meet " + announceIp + " 6379; sleep 15";
